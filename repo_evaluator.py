...
class RepoEvaluator:
    def __init__(self, repo_path: str, owner: str, repo_name: str,
                 platform_client: Optional[PlatformClient],
    ...
    def evaluate(self) -> AnalysisReport:
        ...
        if self.platform_client:
            ...
            pr_analyzer = PRAnalyzer(
                ...
                skip_f2p=self.skip_f2p,
                f2p_timeout=self.f2p_timeout
            )
        ...
        criteria_scores = self._calculate_criteria_scores(repo_metrics, pr_analysis)
    ...
    def _calculate_criteria_scores(self, repo_metrics: RepoMetrics, pr_analysis: PRRejectionStats) -> Dict[str, float]:
        """
        Calculate scores for individual criteria (0-100 scale).
        """
        scores = {}
        
        # 1. Test coverage score
        if repo_metrics.test_coverage_percentage is not None:
            scores['test_coverage'] = min(100, repo_metrics.test_coverage_percentage)
        else:
            # Use test file ratio as fallback (scale to 0-100)
            scores['test_coverage'] = min(100, repo_metrics.test_file_ratio * 1000)  # Rough scaling
        
        # 2. CI/CD score
        scores['ci_cd'] = 100 if repo_metrics.has_ci_cd else 0
        
        # 3. Test runner score
        scores['test_runner'] = 100 if repo_metrics.has_test_runner else 0
        
        # 4. Git activity score (recent commits)
        recent_commits = repo_metrics.recent_commits_12mo or 0
        scores['git_activity'] = min(100, recent_commits * 10)  # Scale: 10 commits = 100
        
        # 5. Issue tracking score
        issue_refs = repo_metrics.commits_referencing_issues or 0
        scores['issue_tracking'] = min(100, issue_refs * 20)  # Scale: 5 refs = 100
        
        # 6. PR acceptance rate score
        if pr_analysis.total_prs > 0:
            scores['pr_acceptance'] = pr_analysis.acceptance_rate * 100
        else:
            scores['pr_acceptance'] = 0
...
# Output functions
def print_report(report: AnalysisReport, high_level: bool = False):
    ...
    for criterion, score in report.criteria_scores.items():
        print(f"{criterion.replace('_', ' ').title()}: {score:.1f}/100")
    print()
...
def clone_repo(repo_full_name: str, temp_dir: Path, token: str, platform: str = 'github') -> Path:
    ...
    git_configs = [
        ...
        ['git', 'config', '--global', 'http.sslVerify', 'false'],  # Try without SSL verification
    ]
    ...
    for config_cmd in git_configs:
        try:
            ...
            pass  # Ignore config timeouts

    # Retry logic for cloning
    max_retries = 3
    for attempt in range(max_retries):
        try:
            logger.info(f"Clone attempt {attempt + 1}/{max_retries}")
            result = subprocess.run(
                # deep clone so we can get the total commits
                ['git', 'clone', '--depth=1', repo_url, str(clone_path)],  # Shallow clone first
                capture_output=True,
                text=True,
            ...
            if result.returncode == 0:
                ...
                if attempt < max_retries - 1:
                    ...
                    raise RuntimeError(f"Failed to clone repository after {max_retries} attempts: {result.stderr}")
            ...
            if clone_path.exists():
                ...
                shutil.rmtree(clone_path, ignore_errors=True)
            if attempt == max_retries - 1:
                raise RuntimeError(f"Failed to clone repository: timeout after {max_retries} attempts")

    # This should not be reached, but just in case
    raise RuntimeError(f"Failed to clone repository: {result.stderr}")


def check_repo_criteria(report: AnalysisReport, filter_level: str = 'good') -> Tuple[bool, List[str]]:
    """
    Check if a repository meets all criteria for good or excellent marks.
    
    Args:
        report: The analysis report for the repository
        filter_level: 'good' or 'excellent' - the minimum level required
    
    Returns:
        Tuple of (passes_filter, list_of_failed_criteria)
    """
    failed_criteria = []
    ...
    if metrics.test_coverage_percentage is not None:
        if filter_level == 'excellent':
            if metrics.test_coverage_percentage < 80:
                failed_criteria.append(f"Test coverage {metrics.test_coverage_percentage:.1f}% < 80%")
        else:  # good
            if metrics.test_coverage_percentage < 60:
                failed_criteria.append(f"Test coverage {metrics.test_coverage_percentage:.1f}% < 60%")
    else:
        # Use test file ratio as fallback
        if filter_level == 'excellent':
            if metrics.test_file_ratio < 0.10:
                failed_criteria.append(f"Test file ratio {metrics.test_file_ratio*100:.1f}% < 10%")
        else:  # good
            if metrics.test_file_ratio < 0.05:
                failed_criteria.append(f"Test file ratio {metrics.test_file_ratio*100:.1f}% < 5%")
    
    # 2. CI/CD check
    if not metrics.has_ci_cd:
        failed_criteria.append("No CI/CD pipeline detected")
    
    # 3. Test runner check
    if not metrics.has_test_runner:
        failed_criteria.append("No test framework detected")
    
    # 4. Git activity check
    recent_commits = metrics.recent_commits_12mo or 0
    if filter_level == 'excellent':
        if recent_commits <= 10:
            failed_criteria.append(f"Recent commits {recent_commits} <= 10")
    else:  # good
        if recent_commits <= 5:
            failed_criteria.append(f"Recent commits {recent_commits} <= 5")
    
    # 5. Issue tracking check
    issue_refs = metrics.commits_referencing_issues
    if filter_level == 'excellent':
        if issue_refs <= 5:
            failed_criteria.append(f"Issue references {issue_refs} <= 5")
    else:  # good
        if issue_refs <= 2:
            failed_criteria.append(f"Issue references {issue_refs} <= 2")
    
    # 6. PR acceptance rate check (for overall score)
    if report.pr_analysis.total_prs > 0:
        acceptance_rate = report.pr_analysis.acceptance_rate
        if filter_level == 'excellent':
            if acceptance_rate < 0.7:
                failed_criteria.append(f"PR acceptance rate {acceptance_rate*100:.1f}% < 70%")
        else:  # good
            if acceptance_rate < 0.5:
                failed_criteria.append(f"PR acceptance rate {acceptance_rate*100:.1f}% < 50%")
    
    passes_filter = len(failed_criteria) == 0
    return passes_filter, failed_criteria
...
def main():
    ...
    parser.add_argument(
        '--filter-good',
        action='store_true',
        help='Only show repositories with good marks for all criteria'
    )
    parser.add_argument(
        '--filter-excellent',
        action='store_true',
        help='Only show repositories with excellent marks for all criteria'
    )
    parser.add_argument(
        '--exclude-poor',
        action='store_true',
        help='Exclude repositories with poor recommendation from output'
    )
    parser.add_argument(
        '--show-all',
        action='store_true',
        help='Show all repositories regardless of recommendation (includes criteria scores)'
    )
    parser.add_argument(
        '--high-level',
        action='store_true',
        help='Show only high-level summary results'
    )
    parser.add_argument(
        '--local-only',
        action='store_true',
        help='Analyze local repository only, skip PR and API analysis'
    )

    args = parser.parse_args()

    # Validate filter arguments
    if args.filter_good and args.filter_excellent:
        logger.error("Cannot use both --filter-good and --filter-excellent at the same time")
        sys.exit(1)
    
    if args.show_all and (args.filter_good or args.filter_excellent or args.exclude_poor):
        logger.error("Cannot use --show-all with other filter flags (--filter-good, --filter-excellent, --exclude-poor)")
        sys.exit(1)
    ...
    try:
        ...
        # Filter recommendations unless --show-all is specified
        if not args.show_all:
            if "FAIR" in report.recommendation or "POOR" in report.recommendation:
                logger.info(f"Repository {args.repo} has {report.recommendation} recommendation. Skipping output.")
                sys.exit(0)

        # Apply filter if specified
        if args.filter_good or args.filter_excellent:
            filter_level = 'excellent' if args.filter_excellent else 'good'
            passes_filter, failed_criteria = check_repo_criteria(report, filter_level)
            
            if not passes_filter:
                logger.info(f"Repository {args.repo} does not meet {filter_level} criteria:")
                for criterion in failed_criteria:
                    logger.info(f"  - {criterion}")
                logger.info("Skipping output for this repository.")
                sys.exit(0)
            else:
                logger.info(f"Repository {args.repo} meets all {filter_level} criteria!")

        # Exclude poor recommendations if specified
        if args.exclude_poor and "POOR" in report.recommendation:
            logger.info(f"Repository {args.repo} has poor recommendation. Skipping output.")
            sys.exit(0)
        ...
        if args.json:
            ...
            print_report(report, args.high_level)
            output = json.dumps(to_json(report), indent=2)
            output_dir = Path(os.getcwd() + '/output')
            os.makedirs(output_dir, exist_ok=True)
            filepath = os.path.join(output_dir, Path(f"{args.repo.replace('/', '__')}.json"))
